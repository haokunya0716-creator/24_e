...
//单位：弧度
volatile static float angle_1 = 0;//大臂角1
volatile static float angle_2 = 0;//大臂角2
volatile static float Angle_1 = 0;//大臂角

volatile static float Angle_2 = 0;//小臂角
...
void Robo_Compute(float x,float y,float z) {
    ...
    if (z > HEIGHT) {
        ...
        Angle_1 = angle_1 + angle_2 + 1.5708;//单位：弧度
    }else if (z < HEIGHT) {
        angle_2 = atan(len_4 / (HEIGHT - z));
