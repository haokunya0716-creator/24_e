...
volatile Emm_V5_Position_t Emm_V5_Position = {0};
...
static __IO Emm_V5_CAN_t Emm_V5_CAN = {0};

static void can_SendCmd(__IO uint8_t *cmd, uint8_t len)
{
  ...
  while(i < j)
  {
    ...
    Emm_V5_CAN.CAN_TxMsg.StdId = 0x00;
    Emm_V5_CAN.CAN_TxMsg.ExtId = ((uint32_t)cmd[0] << 8) | (uint32_t)packNum;
    Emm_V5_CAN.txData[0] = cmd[1];
    Emm_V5_CAN.CAN_TxMsg.IDE = CAN_ID_EXT;
    Emm_V5_CAN.CAN_TxMsg.RTR = CAN_RTR_DATA;

    if(k < 8U)
    {
      for(l = 0; l < k; l++, i++)
      {
        Emm_V5_CAN.txData[l + 1] = cmd[i + 2];
      }
      Emm_V5_CAN.CAN_TxMsg.DLC = k + 1U;
    }
    else
    {
      for(l = 0; l < 7U; l++, i++)
      ...
      Emm_V5_CAN.CAN_TxMsg.DLC = 8U;
    }

    while(HAL_CAN_AddTxMessage((&hcan2), (CAN_TxHeaderTypeDef *)(&Emm_V5_CAN.CAN_TxMsg), (uint8_t *)(&Emm_V5_CAN.txData), (&TxMailbox)) != HAL_OK);
...
void Emm_V5_SetPositionDeg(uint8_t addr, float angle_deg, uint16_t vel)
{
  uint32_t clk;
  float pulses;
  uint8_t dir;

  pulses = ABS(angle_deg) * EMM_V5_PULSES_PER_REV / 360.0f;
  ...
  {
    clk = (uint32_t)(pulses + 0.5f);
  }

  dir = (angle_deg < 0.0f) ? 1U : 0U;
  Emm_V5_Pos_Control(addr, dir, vel, EMM_V5_DEFAULT_ACC, clk, true, false);
}

void Emm_V5_SetPositionRad(uint8_t addr, float angle_rad, uint16_t vel)
{
  Emm_V5_SetPositionDeg(addr, angle_rad * 180.0f / 3.14159265358979323846f, vel);
}

void Emm_V5_SetPosition(uint8_t addr, float angle_rad, uint16_t vel)
{
  Emm_V5_SetPositionRad(addr, angle_rad, vel);
}
...
/**
  * @brief    读取电机实时位置
  * @param    addr  ：电机地址
  * @retval   发送 Addr + 0x36 + 0x6B，电机会通过CAN返回实时位置
  */
void Emm_V5_RequestPosition(uint8_t addr)
{
...
/**
  * @brief    更新电机实时位置
  * @param    addr   ：电机地址
  * @param    ext_id ：CAN扩展帧ID
  * @param    data   ：CAN接收数据
  * @param    len    ：CAN接收数据长度
  * @retval   true为解析到实时位置，false为不是实时位置回包
  */
bool Emm_V5_UpdatePosition(uint8_t addr, uint32_t ext_id, const uint8_t *data, uint8_t len)
{
  uint32_t raw;
  float signed_raw;
  float angle_deg;
  ...
  if((((uint8_t)(ext_id >> 8)) != addr) || (data[0] != 0x36U) || (data[6] != 0x6BU))
  {
    return false;
  }

  raw = ((uint32_t)data[2] << 24) |
        ((uint32_t)data[3] << 16) |
        ((uint32_t)data[4] << 8)  |
        ((uint32_t)data[5]);

  signed_raw = (data[1] != 0U) ? -(float)raw : (float)raw;
  angle_deg = signed_raw * 360.0f / EMM_V5_ENCODER_PER_REV;

  Emm_V5_Position.addr = addr;
  Emm_V5_Position.sign = data[1];
  Emm_V5_Position.raw = raw;
  Emm_V5_Position.angle_deg = angle_deg;
  Emm_V5_Position.angle_rad = angle_deg * 3.14159265358979323846f / 180.0f;
  Emm_V5_Position.updated = true;
...
/**
  * @brief    绝对角度位置模式控制 - 加载到多电机指令上
  * @param    addr      ：电机地址
  * @param    angle_deg ：绝对位置角度，单位为度，正数为CW，负数为CCW
  * @param    vel       ：速度(RPM)，范围0 - 3000RPM
  * @param    acc       ：加速度，范围0 - 255，注意：0是直接启动
  * @param    snF       ：多机同步标志，false为不启用，true为启用
  * @retval   将角度换算为脉冲数后，把位置模式命令加载到多电机命令中
  */
void Emm_V5_MMCL_SetPositionDeg(uint8_t addr, float angle_deg, uint16_t vel, uint8_t acc, bool snF)
{
  uint32_t clk;
  float pulses;
  uint8_t dir;
  ...
  dir = (angle_deg < 0.0f) ? 1U : 0U;
  Emm_V5_MMCL_Pos_Control(addr, dir, vel, acc, clk, true, snF);
}
