...
void QD4310_Update(QD4310_t *dev, const uint8_t feedback[8]) {
    ...
    int16_t current_raw = (feedback[3] << 8) | feedback[2];//电流
    int16_t speed_raw   = (feedback[5] << 8) | feedback[4];//转速
    uint16_t angle_raw  = (feedback[7] << 8) | feedback[6];//角度
